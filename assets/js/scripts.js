...
continue_btn.onclick = ()=>{
    ...
    showQuetions(0); //call showQestions function
    queCounter(1); //pass 1 parameter to queCounter
    startTimer(10); //call startTimer function
...
restart_quiz.onclick = ()=>{
    ...
    userScore = 0;
    widthValue = 0;
    showQuetions(que_count); //call showQestions function
...
next_btn.onclick = ()=>{
    if(que_count < questions.length - 1){ //if question count is less than total question length
        que_count++; //increase the que_count value
        que_numb++; //increase the que_numb value
        showQuetions(que_count); //call showQestions function
        queCounter(que_numb); //pass que_numb value to queCounter
        clearInterval(counter); //clear counter
        clearInterval(counterLine); //clear counterLine
...
// createg the new div tags for  correct and incorrect icons
...
function showResult(){
    ...
    else if(userScore > 7){ // if user scored more than 7
        let scoreTag = '<span>and nice 😎, You got <p>'+ userScore +'</p> out of <p>'+ questions.length +'</p></span>';
        scoreText.innerHTML = scoreTag;
    }
    else{ // if user scored less than 7
